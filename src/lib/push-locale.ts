...
type PushStrings = {
  ...
  sessionUpdated: (sessionName: string) => { title: string; body: string };
  playerCancelled: (playerName: string, sessionName: string) => { title: string; body: string };
  playerJoined: (playerName: string, sessionName: string, confirmed: number, max: number) => { title: string; body: string };
...
export const pushCopy: Record<PushLang, PushStrings> = {
  en: {
    ...
    sessionUpdated: (s) => ({ title: "Session updated", body: `${s} has been updated. Tap to see the changes.` }),
    playerCancelled: (p, s) => ({ title: "Spot freed up", body: `${p} cancelled their spot for ${s}` }),
    playerJoined: (p, s, confirmed, max) => ({ title: "New player joined", body: `${p} joins ${s}, ${confirmed}/${max}` }),
  ...
  vi: {
    ...
    sessionUpdated: (s) => ({ title: "Buổi chơi đã được cập nhật", body: `${s} đã được cập nhật. Nhấn để xem thay đổi.` }),
    playerCancelled: (p, s) => ({ title: "Có chỗ trống", body: `${p} đã hủy chỗ trong ${s}` }),
    playerJoined: (p, s, confirmed, max) => ({ title: "Người chơi mới tham gia", body: `${p} tham gia ${s}, ${confirmed}/${max}` }),
  ...
  ms: {
    ...
    sessionUpdated: (s) => ({ title: "Sesi dikemaskini", body: `${s} telah dikemaskini. Ketik untuk melihat perubahan.` }),
    playerCancelled: (p, s) => ({ title: "Tempat dikosongkan", body: `${p} membatalkan tempat dalam ${s}` }),
    playerJoined: (p, s, confirmed, max) => ({ title: "Pemain baru menyertai", body: `${p} menyertai ${s}, ${confirmed}/${max}` }),
  ...
  fr: {
    ...
    sessionUpdated: (s) => ({ title: "Session modifiée", body: `${s} a été mise à jour. Appuyez pour voir les changements.` }),
    playerCancelled: (p, s) => ({ title: "Place libérée", body: `${p} a annulé sa place dans ${s}` }),
    playerJoined: (p, s, confirmed, max) => ({ title: "Nouveau joueur inscrit", body: `${p} rejoint ${s}, ${confirmed}/${max}` }),
